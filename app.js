...
global.readGuildList = function () {
    ...
    for (let i = 0; i < split.length - 1; i++) { // minus 1 because an extra \n is at the end of the file.
        ...
        // name:owner:public:invite
        // Fallback for old format
        const invite = splitGuild.length > 3 ? splitGuild[3] : `AdminInviteKey`;
        guildList[splitGuild[0]] = { owner: splitGuild[1], public: splitGuild[2], invite };
    }
};
...
global.writeGuildList = function () {
    ...
    for (const i in guildList) {
        ...
        writeStr += `${i}:${guildData.owner}:${guildData.public}:${guildData.invite}\n`;
    }
        ...
        for (let x = 0; x < mapSz; x++) {
            ...
            for (const i in players[y][x]) {
                ...
                if (need_update && cloak) {
                    player.socket.emit(`update`, { disguise: player.disguise, isLocked: player.isLocked, planetTimer: player.planetTimer, charge: player.charge, energy: player.energy, state: { players: [{ delta, id: i }] } });
                    continue;
                }

                if (!need_update) continue;

                gameState.players.push({ delta, id: i });
            }
            ...
            for (const i in vorts[y][x]) {
                ...
                if (pack === undefined) {
                    ...
                    // Send create
                    sendAllSector(`vort_create`, { pack, id: i }, x, y);
                    continue;
                }
                ...
                gameState.vorts.push({ delta, id: i });
            }
            ...
            for (const i in mines[y][x]) {
                ...
                if (pack === undefined) {
                    ...
                    // Send create
                    sendAllSector(`mine_create`, { pack, id: i }, x, y);
                    continue;
                }
                ...
                if (!need_update) continue;
                gameState.mines.push({ delta, id: i });
            }
            ...
            if (tick % 5 == 0) {
                for (const i in packs[y][x]) {
                    ...
                    if (pack === undefined) {
                        ...
                        // Send create
                        sendAllSector(`pack_create`, { pack, id: i }, x, y);
                        continue;
                    }
                    ...
                    if (!need_update) continue;
                    gameState.packs.push({ delta, id: i });
                }
            }
            ...
            for (const i in beams[y][x]) {
                ...
                if (pack == undefined) {
                    ...
                    // Send create
                    sendAllSector(`beam_create`, { pack, id: i }, x, y);
                    continue;
                }
                ...
                if (!need_update) continue;
                gameState.beams.push({ delta, id: i });
            }
            ...
            for (const i in blasts[y][x]) {
                ...
                if (pack === undefined) {
                    ...
                    sendAllSector(`blast_create`, { pack, id: i }, x, y);
                    continue;
                }
                ...
                if (!need_update) continue;
                gameState.blasts.push({ delta, id: i });
            }
            ...
            for (const i in asts[y][x]) {
                ...
                gameState.asteroids.push({ delta, id: i });
            }
            astCount[y][x] = astCt;
            ...
            for (const j in orbs[y][x]) {
                ...
                if (pack === undefined) {
                    ...
                    sendAllSector(`orb_create`, { pack, id: j }, x, y);
                ...
                gameState.orbs.push({ delta, id: j });
            }
            ...
            for (const j in missiles[y][x]) {
                ...
                if (pack === undefined) {
                    ...
                    sendAllSector(`missile_create`, { pack, id: j }, x, y);
                    continue;
                }
                ...
                gameState.missiles.push({ delta, id: j });
            }
...
function updateHeatmap () {
    ...
    for (const i in lb) {
        ...
        lb[i].socket.emit(`heatmap`, { myGuild, hmap, lb: lbSend, youi: i, raidBlue, raidRed, raidGreen });
    }
}
